...
$(function () {
    //搜索关键词
$("#seachkeywords").val("请输入关键词查询").css({ color: "#666" });
    $("#seachkeywords").click(function () {
        $(this).val('').css({ color: "#000" });
    }).blur(function () {
        if ($.trim($(this).val()) == "") {
            $(this).val("请输入关键词查询").css({ color: "#666" });
        }
    });
});
...
$(function() {
    ...
    $.post("/ajax.ashx?action=hotline&t=" + Math.random(), {
        domain: domain
    }, function(msg) {
    ...
    var subtype = "hotline";
    $.post("/ajax.ashx?action=GetSubinfoso&t=" + Math.random(), {
        domain: domain,
        subtype: subtype
    }, function(msg) {
        if (gav(msg, "state") == "1") {
