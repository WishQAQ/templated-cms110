...
class ChannelUnit
{
     ...
     * @return    string
     */
    function GetlitImgLinks($fvalue,$ArcID)
    {
        ...
        foreach($dtp->CTags as $ctag){
            if($ctag->GetName()=="img")
            {
				...
				$ddimg1 = substr($ddimg,0,-7);
				$ddimg2 = substr($ddimg,-4);
				$ddimg3 = $ddimg1.$ddimg2;		
				if($photoid==1) $display="class='zoomThumbActive'"; else $display="";		
				$revalue .= "<li><a $display href='javascript:void(0);' rel=\"{gallery: 'gal1', smallimage: '$ddimg',largeimage: '$ddimg3'}\"><img src='$ddimg'></a></li>";
	...
	function GetBigImg($fvalue,$ArcID)
    {
        if($GLOBALS["htmltype"]=="dm"){
            if(empty($GLOBALS["pageno"])) $NowPage = 1;
            else $NowPage = intval($GLOBALS["pageno"]);
        ...
        foreach($dtp->CTags as $ctag){
            if($ctag->GetName()=="img")
            {
				...
				$ddimg2 = substr($ddimg,-4);
				$ddimg = $ddimg1.$ddimg2;
				if($photoid==1) $display="selected='selected'"; else $display="";
				$revalue .="<bigpic $display>$ddimg</bigpic>";
	...
	function GetSmallImg($fvalue,$ArcID)
    {
        if($GLOBALS["htmltype"]=="dm"){
            if(empty($GLOBALS["pageno"])) $NowPage = 1;
            else $NowPage = intval($GLOBALS["pageno"]);
        ...
        foreach($dtp->CTags as $ctag){
            if($ctag->GetName()=="img")
            {
				...
				//$ddimg = $ddimg1.$ddimg2;
				if($photoid==1) $display="selected='selected'"; else $display="";				
				$revalue .="<smallpic $display>$ddimg</smallpic>";
	...
	function GetFirstImg($fvalue,$ArcID)
    {
        if($GLOBALS["htmltype"]=="dm"){
            if(empty($GLOBALS["pageno"])) $NowPage = 1;
            else $NowPage = intval($GLOBALS["pageno"]);
        ...
        foreach($dtp->CTags as $ctag){
            if($ctag->GetName()=="img")
            {
				...
				$ddimg1 = substr($ddimg,0,-7);
				$ddimg2 = substr($ddimg,-4);
				$ddimg_big = $ddimg1.$ddimg2;
				
				
      $revalue .="<a class='jqzoom' href='$ddimg_big' > <img src='$ddimg_big'  class='js_goods_image_url' style='height: 294px; width: 340px;'></a> ";		
